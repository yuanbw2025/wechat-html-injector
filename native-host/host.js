...
async function clip(payload) {
  ...
  const target = await locateTarget(payload.targetUrl);
  ...
  if (target.kind === 'wiki') {
    ...
    return { ok: true, link: url, message: `已创建并验证 WPS 知识库文档（${payload.images?.length || 0} 张图片）` };
  }
  const created = await runCli('drive', 'create-file-with-content', { ...target, name: `${safeTitle}.otl`, file_extension: 'otl', content: payload.markdown });
  ...
  return { ok: true, link: file.link_url || file.url || '', message: `已创建并验证 WPS 文档（${payload.images?.length || 0} 张图片）` };
}
