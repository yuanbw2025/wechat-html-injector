...
普通用户不需要操作本目录。点击浏览器工具栏中的插件图标，在唯一的插件设置页切换到“WPS 剪存”，即可下载安装器、检测组件并填写 WPS 目标目录。macOS 安装器以原生 `.pkg` 提供，双击后由系统安装器完成全部步骤，不需要执行命令、解压文件或打开终端。安装器会自动下载 `kdocs-cli` 和 Node.js、写入本地组件、注册 Chrome/Edge Native Messaging，并打开 WPS 登录。
